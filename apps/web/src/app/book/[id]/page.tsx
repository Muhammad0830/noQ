...
export default function BookingPage({
  ...
  const [selectedServiceId, setSelectedServiceId] = useState<string | null>(serviceFromQuery ?? null)
  ...
  const { data: shop } = useApiQuery<Shop>(API_ENDPOINTS.shopById(shopId), { key: ['shop', shopId] })
  const { data: services = [], isLoading: servicesLoading } = useApiQuery<Service[]>(API_ENDPOINTS.shopServices(shopId), {
    key: ['services', shopId],
  })
  const { data: staffData = [] } = useApiQuery<Staff[]>(`${API_ENDPOINTS.shopById(shopId)}/staff`, {
    key: ['staff', shopId],
  })
  ...
  const { data: slotsData = [] } = useApiQuery<TimeSlot[]>(
    effectiveDate ? `${API_ENDPOINTS.bookings}/available-slots?shopId=${shopId}&date=${effectiveDate}` : null,
  ...
  const selectedStaffMember = staff.find((member) => member.id === selectedStaff) ?? staff[0] ?? null
  const selectedDateObj = nextDays.find((d) => d.toISOString().split('T')[0] === effectiveDate) ?? nextDays[0]
  const duration = selectedService?.durationMin ?? 45
  const bookingEndTime = selectedTime ? addMinutes(selectedTime, duration) : null
  ...
  const minuteToTop = (minute: number) => ((minute - dayStartMin) / (dayEndMin - dayStartMin)) * timelineHeight
  ...
  const visibleDays = useMemo(() => nextDays.slice(visibleDayStart, visibleDayStart + 5), [nextDays, visibleDayStart])

  const timelineSlots = useMemo(
    () =>
      timeSlots
        .map((slot) => ({ ...slot, startMin: timeToMinutes(slot.time) }))
        .filter((slot) => slot.startMin >= dayStartMin && slot.startMin <= dayEndMin)
      ...
      <div className="max-w-md mx-auto px-4 pt-3">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            <p className="text-sm font-semibold tracking-wide">{t('booking.title')}</p>
            <p className="text-[10px] text-cyan-600 dark:text-teal-400 uppercase tracking-[0.2em]">{shop?.name || 'Luxe Studio'}</p>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2 mt-1 text-xs">
                  <span className="text-cyan-600 dark:text-teal-400 font-semibold">${selectedService.price ?? 0}</span>
                  <span className="text-slate-400 dark:text-gray-500">·</span>
                  <span className="text-slate-500 dark:text-gray-400">{duration} min</span>
                </div>
              </div>
              <button
          ...
          <div className="space-y-2 mb-6">
            {services.map((service) => (
              ...
              >
                ...
                <p className="text-xs text-slate-500 dark:text-gray-400 mt-1">${service.price ?? 0} · {service.durationMin ?? 45} min</p>
              </button>
            ))}
          </div>
        )}
        ...
        <section className="mb-6">
          <h3 className="text-sm font-semibold mb-3 text-slate-800 dark:text-gray-200">{t('booking.selectStaff')}</h3>
          <div className="flex items-start gap-4 overflow-x-auto pb-2">
            {staff.map((member) => {
              const isActive = (selectedStaff ?? selectedStaffMember?.id) === member.id
              return (
                <button key={member.id} onClick={() => setSelectedStaff(member.id)} className="shrink-0 text-center">
                  <div
                  ...
                  <p className={`mt-2 text-[10px] ${isActive ? 'text-cyan-600 dark:text-teal-400' : 'text-slate-500 dark:text-gray-400'}`}>{member.name}</p>
                </button>
              )
            })}
          </div>
        ...
        <section className="mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => setVisibleDayStart((prev) => Math.min(Math.max(nextDays.length - 5, 0), prev + 5))}
                className="h-7 w-7 rounded-full border border-slate-300 dark:border-gray-700 flex items-center justify-center text-slate-500 dark:text-gray-400"
              >
          ...
          <div className="grid grid-cols-5 gap-2">
            {visibleDays.map((day) => {
              ...
              return (
                ...
                >
                  <p className="text-[10px] uppercase">{weekdayShortMap[language][day.getDay()]}</p>
                  <p className="text-lg font-semibold leading-5 mt-1">{day.getDate()}</p>
                </button>
              )
            })}
          </div>
        ...
        <section className="mb-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-[10px] px-2 py-1 rounded-full bg-cyan-100 dark:bg-teal-500/20 border border-cyan-400/30 text-cyan-700 dark:text-teal-400">{t('booking.liveSelection')}</div>
          </div>

          <div className="rounded-2xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
            <div className="flex gap-3">
              <div className="w-12 shrink-0 relative" style={{ height: `${timelineHeight}px` }}>
                {hourLabels.map((hour) => {
                  const top = minuteToTop(timeToMinutes(hour))
                  return (
                    <span key={hour} className="absolute -translate-y-2 text-[10px] text-slate-500 dark:text-gray-400" style={{ top }}>
                      {hour}
                    </span>
                  )
                })}
              ...
              <div className="relative flex-1 rounded-xl border border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-900" style={{ height: `${timelineHeight}px` }}>
                {hourLabels.map((hour) => {
                  const top = minuteToTop(timeToMinutes(hour))
                  return <div key={hour} className="absolute left-0 right-0 border-t border-dashed border-slate-200 dark:border-gray-700" style={{ top }} />
                })}

                {timelineSlots.map((slot) => {
                  const active = selectedTime === slot.time
                  const top = minuteToTop(slot.startMin)
                  const height = Math.max((duration / (dayEndMin - dayStartMin)) * timelineHeight, 36)
                  ...
                  return (
                    ...
                    >
                      <div className="h-full flex flex-col justify-center">
                        <span className="text-xs font-medium">{slot.time} {slot.available ? `– ${addMinutes(slot.time, duration)}` : ''}</span>
                        <span className="text-[10px] uppercase opacity-70">{slot.available ? t('booking.availableSlot') : t('booking.alreadyReserved')}</span>
                      </div>
                    </button>
                  )
                })}
        ...
        <div className="max-w-md mx-auto p-4 rounded-2xl bg-white/90 dark:bg-gray-900 backdrop-blur-md border border-cyan-200 dark:border-gray-700 shadow-lg">
          <div className="rounded-2xl border border-cyan-300/50 dark:border-gray-700 bg-linear-to-r from-cyan-50 to-emerald-50 dark:from-gray-800 dark:to-gray-800 p-3 mb-3">
            <div className="flex justify-between text-xs text-slate-600 dark:text-gray-300">
              <span className="uppercase tracking-wide">{t('booking.selectedWindow')}</span>
              <span className="uppercase tracking-wide">{t('booking.total')}</span>
            </div>
            <div className="flex justify-between items-end mt-1">
              <p className="font-semibold text-cyan-700 dark:text-teal-400">
                {selectedTime && bookingEndTime ? `${selectedTime} — ${bookingEndTime}` : '--:--'}
              </p>
              <p className="text-2xl font-bold text-cyan-700 dark:text-teal-400">${totalPrice}</p>
            </div>
          </div>

          <button
            onClick={handleConfirmBooking}
            disabled={!selectedService || !selectedTime || bookingMutation.isPending}
          ...
          >
            {bookingMutation.isPending ? t('booking.processing') : t('booking.confirmBooking')}
          </button>
